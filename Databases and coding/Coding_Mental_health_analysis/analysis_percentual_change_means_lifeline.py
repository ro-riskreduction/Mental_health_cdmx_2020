...
for (d0a,d0b),(d1a,d1b) in date_pairs:
    ...
    for name in names:
        ...
        mean_post_0 = np.mean(y1)
        mean_post_0 = np.around(mean_post_0, 3)
        mean_post.append(mean_post_0)
        #r     = compare_mean_max(y0, y1)
        print( f' pre: {d0a},{d0b}  ;  post: {d1a},{d1b}' )
        print( f'    {name}'  )
        print(mean_pre_0, mean_post_0)
    print('\n\n\n')
    
mean_pre_array = np.asarray(mean_pre)
mean_post_array = np.asarray(mean_post)

#flatten
mean_pre_array = mean_pre_array.flatten()
mean_post_array = mean_post_array.flatten()

print("mean_pre_array")
print(mean_pre_array)
print("mean_post_array")
print(mean_post_array)

print("compare_mean_max")
compare_mean_max(mean_pre_array, mean_post_array)
